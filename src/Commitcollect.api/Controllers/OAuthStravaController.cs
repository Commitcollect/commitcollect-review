...
public class OAuthStravaController : ControllerBase
{
        ...
        [FromQuery] string? scope)
    {
        ...
        try
        {
            ...
            // STRAVA_AUTHORIZE — durable success only (token + uniqueness lock + connection write succeeded)

                            await _audit.TryWriteAsync(
                    HttpContext,
                    userId: userId,
                    eventType: "STRAVA_AUTHORIZE",
                    result: "success",
                    data: new Dictionary<string, AttributeValue>
                    {
                        ["athleteId"] = new AttributeValue { N = (token.Athlete?.Id ?? 0).ToString() },
        ...
        if (string.IsNullOrWhiteSpace(frontendBaseUrl))
            frontendBaseUrl = "https://app.commitcollect.com"; // safe default

        var redirectUrl = $"{frontendBaseUrl.TrimEnd('/')}/app?strava=connected" +
